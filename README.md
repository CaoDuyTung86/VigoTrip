...
- [ ] **Kho tri thức tiếng Anh:** 56 chunk hiện có đều là `lang: vi`, và nhánh BM25 khớp theo mặt chữ nên câu hỏi tiếng Anh gần như không chạm được vào chunk nào. Đo thử tám câu hỏi tiếng Anh thường gặp: recall@3 của nhánh từ khoá là 2/8, trong đó *"my discount code is not working"* trả về **rỗng hoàn toàn** dù kho có hẳn chunk `voucher-not-working`; cùng bộ đo trên câu hỏi tiếng Việt là 0.85 trở lên. Khách hỏi tiếng Anh vì thế đang dựa hết vào nhánh ngữ nghĩa, và khi thiếu khoá embedding thì nhánh đó cũng tắt — tức là không còn gì. Việc cần làm là một file `faq-en.yml` viết song song, dùng LẠI đúng `docId` của bản tiếng Việt để bộ đo và phần quản trị không phải tách đôi. Kèm theo: thêm câu hỏi tiếng Anh vào `rag-eval.yml`, vì thêm nội dung mà không thêm phép đo thì không biết mình có tiến bộ thật hay không.
- [ ] **Bộ đo chọn tool:** Phần truy hồi có `RagRetrievalQualityTest` chốt recall@3 và MRR, còn việc model có gọi ĐÚNG tool hay không thì hiện chỉ biết bằng cách thử tay. Một bộ ca hỏi kèm tool đúng kỳ vọng là thứ giữ cho tool thứ bảy trở đi không làm hỏng tool thứ nhất — đặc biệt sau khi vòng lặp thành nhiều lượt, vì giờ model còn phải chọn đúng THỨ TỰ chứ không chỉ đúng tool.
- [ ] **Tool có ghi dữ liệu:** Cả sáu tool hiện có đều chỉ đọc. Bước sau là một hành động thật, ví dụ lưu mã giảm giá vào tài khoản, kèm một bước xác nhận rõ ràng trước khi ghi. *Phải xử lý trước:* `LlmRouter.execute` chạy LẠI toàn bộ vòng function calling trên nhà cung cấp kế tiếp khi failover — vô hại với truy vấn chỉ đọc, nhưng với tool ghi thì đó là ghi hai lần. Riêng đặt vé qua chat thì không nên: luồng chọn ghế và thanh toán không nén vào hội thoại được mà không mất kiểm soát.
