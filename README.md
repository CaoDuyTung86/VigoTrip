...
- [ ] **Tool thời tiết cho chatbot:** Cho AI gọi được dự báo, để câu "cuối tuần này đi Đà Nẵng thời tiết sao" có số liệu thật thay vì một câu chung chung. Hạ tầng đã sẵn: `AIService` có vòng function calling và đang chạy năm tool, thêm một tool nữa là một định nghĩa trong `buildToolsDefinition` cộng một nhánh trong `ChatService.executeTool` gọi `WeatherService.forecastFor`. *Điều kiện cần:* `PlaceCatalog` hiện chỉ tra theo mã điểm, phải thêm đường tra theo tên tiếng Việt/tiếng Anh — người dùng gõ "Đà Nẵng" chứ không gõ `DAD`, và để model tự đoán mã là để nó đoán sai. *Bắt buộc kèm theo:* một câu trong system prompt cấm suy ra khả năng hoãn/huỷ chuyến từ thời tiết. Khối thời tiết trên giao diện đã có dòng tách hai thứ đó, còn chatbot thì chưa có gì tương đương — mà model rất sẵn lòng nối "mưa to" với "chuyến có thể trễ", và dự báo sai lúc đó thành khiếu nại về tiền. Ngoài tầm bảy ngày thì tool trả rỗng và model phải nói thẳng là chưa có dự báo, không được bịa.
