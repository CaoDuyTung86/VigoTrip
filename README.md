...
- **AI Chatbot với Hybrid RAG:** Tri thức lưu trong CSDL kèm vector embedding, truy hồi lai giữa tìm kiếm ngữ nghĩa (cosine) và BM25 — nhánh vector là nhánh chính, BM25 chỉ lấp chỗ trống khi vector trả chưa đủ topK. Kho tri thức song song bốn ngôn ngữ (224 chunk). Đo trên bộ 132 câu hỏi vàng `vi`/`en`/`ja`/`zh`, đường production (hybrid + lọc ngôn ngữ): **P@1 95.5%, recall@3 99.2%, MRR 0.975**. Không có embedding thì tự rơi về BM25 thuần: P@1 76.5%, recall@3 95.5%, MRR 0.861.
...
- [x] **Advanced RAG & Hybrid Search:** Knowledge base chuyển từ hằng số hardcode sang bảng `tri_thuc` trong CSDL, mỗi chunk kèm vector embedding. Truy hồi lai vector + BM25, tự suy giảm êm về BM25 thuần khi thiếu API key. Có API quản trị tri thức cho Admin và bộ đo chất lượng truy hồi tự động (P@1, recall@3, MRR). **Cách hợp nhất đã đổi ngày 14/09/2026:** trước đó hai nhánh gộp bằng Reciprocal Rank Fusion; nay kết quả giữ nguyên thứ tự của nhánh vector và BM25 chỉ lấp phần còn thiếu, không cộng điểm và không bao giờ đẩy một chunk lên trên chunk mà vector đã xếp. Lý do bỏ RRF: trên bộ vàng hai cách ngang nhau, nhưng trên câu hỏi chưa dùng để chọn cấu hình thì RRF thua — chunk sai có mặt ở **cả hai** nhánh cộng điểm vượt được chunk đúng mà chỉ vector xếp đầu. P@1 tiếng Việt + lọc lang: 49 câu người thật gõ 63.3% → 73.5%, 55 câu holdout 80.0% → 94.5%, 32 câu có mã voucher/mã đơn 81.2% → 90.6%.
...
- [x] **Kho tri thức đa ngôn ngữ (`en`, rồi cả `ja`/`zh`):** Trước đây 56 chunk đều là `lang: vi`, nhánh BM25 khớp theo mặt chữ nên câu hỏi tiếng Anh gần như không chạm được chunk nào — tám câu thử chỉ trúng 2, và *"my discount code is not working"* trả về rỗng dù kho có hẳn chunk `voucher-not-working`. Nay có `faq-en.yml`, `faq-ja.yml`, `faq-zh.yml` viết song song, mỗi mục ứng đúng một mục tiếng Việt. Ba thay đổi đi kèm, mỗi cái đều do bộ đo chỉ ra chứ không phải đoán: **lọc theo `lang`** ở cả hai nhánh truy hồi (`LangFilter`) để chunk tiếng Việt không chen vào top của câu hỏi tiếng Anh; **bảng từ đồng nghĩa riêng theo ngôn ngữ**, vì bảng chung nhét cả loạt từ về thú cưng vào câu hỏi chọn ghế; và **thống kê BM25 tách theo ngôn ngữ**, vì chunk tiếng Nhật/Trung dài hơn làm lệch độ dài trung bình và kéo tụt R@3 tiếng Việt. Tiếng Nhật/Trung tách token bằng bigram chữ Hán/kana, không thì chunk vô hình với BM25. Bộ câu hỏi vàng lên 132 câu (`vi` 59, `en` 37, `ja` 18, `zh` 18), chấm và chốt ngưỡng riêng từng ngôn ngữ (R@3 ≥ 0.85, MRR ≥ 0.70). Ngưỡng này gác **nhánh BM25 chạy offline** — nhánh duy nhất CI đo được mà không cần API key: `vi` 94.9% / 0.843, `en` 94.6% / 0.914, `ja` 94.4% / 0.844, `zh` 100% / 0.824. Đường production có thêm nhánh vector nên cao hơn hẳn (xem mục Tính Năng Nổi Bật). Còn nợ: `faq-ja.yml`/`faq-zh.yml` chưa qua tay người bản ngữ, và `ja`/`zh` mới có 18 câu hỏi vàng mỗi thứ tiếng — quá ít để con số 94.4% và 100% nói lên được nhiều. Chi tiết tại [docs/CHATBOT_AI.md](./docs/CHATBOT_AI.md) mục 10.

- [x] **Bộ đo chọn tool:** `tool-eval.yml` (53 ca — `vi` 29, `en` 12, `ja` 6, `zh` 6) + `ToolSelectionQualityTest`. Chọn tool là bài phân loại nhiều nhãn nên dùng precision/recall/F1 nguyên bản, cộng thêm tỉ lệ khớp bộ, gọi thừa, bỏ tra và khớp tham số. 14 ca **không được** gọi tool nào — thiếu nửa này thì bộ đo sẽ khen một hệ thống gọi tool cho mọi thứ. Trường `forbid` chấm riêng lỗi bịa tham số, sinh ra từ vụ `QNH` bị hiểu thành Quy Nhơn. Bộ đo gọi thẳng `AIService.getChatResponse` và bắt lại định nghĩa tool mà nó gửi đi, nên không có bản sao nào lệch pha với code thật. Phần offline chạy trong mọi lượt test: tool đổi tên, tham số bị bỏ, hay thêm tool mới mà chưa có ca đo đều làm build đỏ. Phần live đo ngày 13/09/2026 trên `gemini-flash-lite-latest`, **trên phiên bản 46 ca của bộ đo lúc đó**: 46/46 khớp bộ, 0/13 gọi thừa, 100% tham số khớp, 2,07 lời gọi mô hình mỗi câu. Bảy ca thêm vào sau đó **chưa được đo live lần nào** — con số 46/46 vì thế nói về bộ đo cũ, không phải bộ đo hiện tại. Điểm tối đa được ghi lại đúng như nó là: tin tốt về hệ thống, tin xấu về cây thước — nó chỉ còn dùng làm chốt chặn chống thoái lui, và chỗ cần mở rộng tiếp là tham số chứ không phải việc chọn tool. Bộ đo tự fail nếu có lời gọi hỏng vì rate limit, vì lời gọi hỏng bị chấm thành "mô hình bỏ tra" — sai mà không báo. Chi tiết tại [docs/CHATBOT_AI.md](./docs/CHATBOT_AI.md) mục 6.8.
