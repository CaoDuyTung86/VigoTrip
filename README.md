...
- [ ] **Bảng tin chạy (Announcement Ticker):** Dải chữ chạy ngang kiểu bảng điện tử nhà ga, đặt trên `Header`, gom các thông tin dùng chung cả site — voucher đang hiệu lực, tuyến mới mở bán, lịch bảo trì. Chỉ dành cho thông tin **không khẩn cấp**: việc giữ ghế sắp hết hạn vẫn thuộc `HoldCountdownBanner`, phản hồi tức thời sau thao tác vẫn thuộc `ToastContext`. Bắt buộc tạm dừng khi rê chuột hoặc focus bàn phím, tôn trọng `prefers-reduced-motion` (thoái lui về danh sách tĩnh), và mỗi mẩu tin phải mở được trang đầy đủ để đọc lại — chữ chạy chỉ là mồi, không phải nơi chứa nội dung.
  - *Không cần WebSocket:* thông báo đổi vài lần mỗi tuần, khác hẳn trạng thái ghế đổi từng giây. Một endpoint công khai đọc danh sách đang hiệu lực, cache ngắn phía máy chủ, client gọi lại khi cửa sổ được focus là đủ. Mở thêm kênh STOMP toàn cục sẽ buộc mọi khách vãng lai giữ kết nối thường trực chỉ để nhận một dải quảng cáo.
  - *Làm hai nhịp:* nhịp một không đụng lược đồ CSDL — suy trực tiếp từ `Voucher` / `Promotion` đang hiệu lực. Nhịp hai mới thêm bảng `thong_bao` kèm trang quản trị, dựng theo khuôn `AdminVouchers`. Trường quan trọng nhất là mốc `hieu_luc_tu` / `hieu_luc_den` để tin tự hết hạn, tránh cảnh dải tin còn treo khuyến mãi Tết vào tháng Tư.
  - *Ngôn ngữ:* chỉ nhập `vi` + `en` theo hiện trạng ưu tiên dịch thuật; thiếu bản tiếng Anh thì hiện tiếng Việt chứ không ẩn tin, còn thiếu bản tiếng Việt là lỗi nhập liệu và phải chặn ngay ở form.
