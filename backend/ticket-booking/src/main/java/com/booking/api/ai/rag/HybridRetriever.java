...
public class HybridRetriever {
    ...
    /** Truy hồi với topK lấy từ cấu hình. */
    ...
    public List<KnowledgeChunk> retrieve(String query, int topK) {
        ...
        try {
            ...
            List<ScoredChunk> lexicalHits = lexicalIndex.search(query, candidates);
            List<ScoredChunk> semanticHits = semanticSearch(query, candidates);
    ...
    private List<ScoredChunk> semanticSearch(String query, int candidates) {
        if (!embeddingClient.isAvailable() || vectorStore.size() == 0) {
            return List.of();
        }
        try {
            float[] queryVector = embeddingClient.embed(query);
            return vectorStore.search(queryVector, candidates, properties.getMinSimilarity());
    ...
    public List<KnowledgeChunk> retrieveLexicalOnly(String query, int topK) {
        return lexicalIndex.search(query, topK).stream().map(ScoredChunk::chunk).toList();
    }
    ...
    public List<KnowledgeChunk> retrieveSemanticOnly(String query, int topK) {
        return semanticSearch(query, topK).stream()
