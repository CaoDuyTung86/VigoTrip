    ...
    return (
      <div key={s.id} style={{ position: 'relative' }}>
        <button
          ...
          className={`seat-btn${isSleeper ? " seat-btn-wide" : ""}`}
          style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'slideUp 0.45s cubic-bezier(0.16, 1, 0.3, 1)' }}>
          ...
          <div style={{
            ...
            <div className="seat-row seat-head" style={{
              ...
              {leftCols.map(c => <div key={c} className="seat-cell" style={{ width: 46, textAlign: 'center', fontWeight: 800, color: 'var(--primary)', fontSize: 13 }}>{c}</div>)}
              <div className="seat-aisle" style={{ width: 44, textAlign: 'center', fontSize: 10, fontWeight: 700, color: 'var(--text-muted)' }}>{t.smAisle}</div>
              {rightCols.map(c => <div key={c} className="seat-cell" style={{ width: 46, textAlign: 'center', fontWeight: 800, color: 'var(--primary)', fontSize: 13 }}>{c}</div>)}
            </div>
              ...
              {toaRows.map(row => {
                ...
                return (
                  <div key={row} className="seat-row" style={{
                    ...
                    <div className="seat-rail" style={{ width: 36, textAlign: 'center', fontWeight: 700, color: rowIsBiz ? 'var(--primary)' : 'var(--text-secondary)', fontSize: 12 }}>{row}</div>
                    {leftCols.map(col => {
                      const s = smap.get(`${row}${col}`);
                      if (!s) return <div key={col} className="seat-cell" style={{ width: 46, height: 44 }} />;
